...
class HexTest extends AnyFlatSpec with ChiselScalatestTester {
    val e32m1Config = () => {
        val cpu = new Top
        cpu.mem.loadMemoryFromHexFile("src/test/resources/hex/vle32m1.hex")
        cpu
    }
    val e64m1Config = () => {
        val cpu = new Top
        cpu.mem.loadMemoryFromHexFile("src/test/resources/hex/vle64m1.hex")
        cpu
    }
    val e32m2Config = () => {
        val cpu = new Top
        cpu.mem.loadMemoryFromHexFile("src/test/resources/hex/vle32m2.hex")
        cpu
    }
    behavior of "cpu"
