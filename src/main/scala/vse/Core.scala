...
class Core extends Module {
    ...
    val controlSignals = ListLookup(inst, List(AluX, Op1Rs1, Op2Rs2, MenX, RenS, WbX, CsrX),  Array(
        ...
        Vse     -> List(AluCopy1,  Op1Rs1, Op2X,   MenV, RenX, WbX,    CsrX)
    ))
    ...
    val vs3Data = Cat(Seq.tabulate(8)(n => vecRegFile(wbAddr + n.U)).reverse)
    ...
    val csrSew  = (1.U(1.W) << (csrVsew+3.U)) // Suppose EEW = SEW
    ...
    val vWbData = Mux(wbSel === WbMemV, dmem.vRdata, vAluOut)
    ...
    printf(cf"dmem.vRdata      : 0x${Hexadecimal(dmem.vRdata)}\n")
